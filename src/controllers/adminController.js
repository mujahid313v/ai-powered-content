...
exports.approveContent = async (req, res) => {
  ...
  const moderatorId = req.user?.id || null;
...
exports.bulkAction = async (req, res) => {
  ...
  try {
    ...
    for (const id of content_ids) {
      ...
      // Emit notification for each item
      await notificationService.notifyModerationCompleted({
        contentId: id,
        decision: newStatus,
        moderatorId
    ...
    // Broadcast queue update after bulk action
    notificationService.broadcastQueueUpdate(await notificationService.getQueueStats());
